...
def evaluate(env: StarCraft2Env, agent: PGLearner, n_trajectories: int = 1) -> InfoDict:
    returns = []
    total_steps = 0
    end_info = {"dead_allies": [],
                "dead_enemies": [],
    ...
    for traj_ind in range(n_trajectories):
        ...
        env.reset()
        while not done:
            observations = np.stack(env.get_obs())
            available_actions = env.get_avail_actions()
            actions = agent.sample_actions(jnp.array(observations),
                                           jnp.array(available_actions),
                                           distribution='det')
...
def main(_):
    ...
    env_info = env.get_env_info()
    dummy_state = np.ones(env_info['state_shape'])
    dummy_observation = np.ones(env_info['obs_shape'])
    ...
    available_actions_sample = jnp.zeros((env_info["n_actions"],), dtype=bool)
    agent = PGLearner(FLAGS.seed,
                      dummy_state[np.newaxis],
                      dummy_observation[np.newaxis],
                      available_actions_sample[np.newaxis],
                      env_info["n_actions"],
